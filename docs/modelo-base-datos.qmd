...
de juego: guarda la fecha de creación y el usuario al que pertenece. `attempts`
registra cada intento, con el nivel jugado, el resultado, el final alcanzado, el
número de intento y la fecha de finalización. Las relaciones son de uno a
muchos: un usuario puede abrir muchas sesiones y una sesión puede acumular
muchos intentos; a la inversa, cada sesión pertenece a un único usuario y cada
intento a una única sesión.
...
de PostgreSQL con los valores `ADMIN`, `USUARIO` y `AUDITOR`. Los campos
`username` y `passwordHash` admiten valor nulo.
